...
def check_toc_format(doc):
    if doc.TablesOfContents.Count > 0:
        ...
        for paragraph in toc.Range.Paragraphs:
            ...
            if issues:
                # 在目录的第一行添加批注
                add_comment(paragraph, issues,"目录")
                break  # 只处理目录的第一行
    else:
        print("文档中未找到目录。")
...
def check_normal_format(doc):
    for para in doc.Paragraphs:
        ...
        if para.Style.NameLocal == '正文':
            issues = check_paragraph_format(para)
            if issues:
...
if __name__ == '__main__':
    ...
    date_format_check(doc)  # 首页日期格式校验
    check_toc_format(doc)   # 目录格式校验
    persons = extract_person_info(doc_path) # 获取分发表人员信息
    print(persons)

    check_table_paragraph_alignment(doc) # 表名格式校验

    check_normal_format(doc)    # 正文内容格式校验
